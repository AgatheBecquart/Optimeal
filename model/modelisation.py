...
def modelisation(connection, run_name, start_date, end_date):
    ...
    experiment_name = (
        "predict_couverts" if run_name != "test_run" else "test_experiment"
    )
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=run_name) as run:
        ...
        y_train_pred = [
            max(0, pred) for pred in y_train_pred
        ...
        rmse_train = round(
            mean_squared_error(y_train, y_train_pred, squared=False), 4
        )  # RMSE
        ...
        rmse_test = round(
            mean_squared_error(y_test, y_test_pred, squared=False), 4
        )  # RMSE
